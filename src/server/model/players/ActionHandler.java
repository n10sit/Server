...
public class ActionHandler {
	...
	public void firstClickObject(int objectType, int obX, int obY) {
		...
		case 2971:
			if (c.absY > 4690) {
				c.getPA().movePlayer(2420, 4690, 0);
			}
			if (c.absY <= 4690) { //in dungeon one
				for (int n = 0; n < NPCHandler.maxNPCs; n++) {
					if (NPCHandler.npcs[n] != null) {
						if (NPCHandler.npcs[n].inDungeonOne()) {
							c.sendMessage("Room 1 is not clear!");
							break;
						} else {
							c.getDH().sendDialogues(78, 1375);
						}
					}
				}
			}
...
c.sendMessage("An Lunar wisdom fills your mind.");
c.getPA().resetAutocast();
} else {
c.setSidebarInterface(6, 1151); //modern
c.playerMagicBook = 0;
...
break; 
	...
	public void firstClickNpc(int npcType) {
		...
		switch(npcType) {
		case 231:
			c.getDH().sendDialogues(79, npcType);
			break;
		case 594:
			c.getDH().sendDialogues(81, npcType);
			break;
		case 582:
		...
		case 1375:
			...
			break;
			case 549:
				c.getDH().sendDialogues(38, npcType);
			break;
			case 518:
				if (c.checkBetTime())
					c.getDH().sendDialogues(65, npcType);
				else
					c.sendMessage("Come back after winning 10 fights!");
			break;
			case 2635:
				if (c.npcRank < 5 && c.pvpRank < 5)
					c.sendMessage("Bob meows at you. You have no idea what he is saying.");
				else
					c.getDH().sendDialogues(61, npcType);
			break;
			case 2636:
				if (c.playerJob < 0) {
					for (int i = 0; i < Server.npcHandler.jobs.length; i++) {
						int jobToGive = Misc.random(Server.npcHandler.jobs[i]);
						if (jobToGive == Server.npcHandler.lastJobGiven) {
							c.playerJob = jobToGive;
							Server.npcHandler.lastJobGiven = c.playerJob;
						} else {
							c.sendMessage("Bob ignored you. Try again.");
							return;
						}
					}
				}
