...
c.lastCast = System.currentTimeMillis();
	...
	**/
	...
	public void killsHSN() {
		int i = 0;
		c.getPStats().doKillsHS(false);
		sendFrame126("Current Champion: "+NPCHandler.npcChampion, 6399);
		for (Object e : c.getPStats().theKillsObject) {
			sendFrame126(1+i+". " + ((Map.Entry<String, Integer>) e).getKey() + " - " + ((Map.Entry<String, Integer>) e).getValue(), 6402+i);
			i++;
		}
		showInterface(6308);
	}
	...
	@SuppressWarnings("unchecked")
	public void deathsHSN() {
		int i = 0;
		c.getPStats().doDeathsHS(false);
		sendFrame126("Current Champion: "+NPCHandler.npcChampion, 6399);
		for (Object e : c.getPStats().theDeathsObject) {
			sendFrame126(1+i+". " + ((Map.Entry<String, Integer>) e).getKey() + " - " + ((Map.Entry<String, Integer>) e).getValue(), 6402+i);
			i++;
		}
		showInterface(6308);
	...
	@SuppressWarnings("unchecked")
	public void killsHSP() {
		for (int x = 6402; x < 6450; x++) {
			sendFrame126("", x);
		}
		int i = 0;
		c.getPStats().doKillsHS(true);
		sendFrame126("Name - Kills", 6399);
		for (Object e : c.getPStats().theKillsObject) {
			sendFrame126(1+i+". " + ((Map.Entry<String, Integer>) e).getKey() + " - " + ((Map.Entry<String, Integer>) e).getValue(), 6402+i);
			i++;
		}
	...
	@SuppressWarnings("unchecked")
	public void deathsHSP() {
		for (int x = 6402; x < 6450; x++) {
			sendFrame126("", x);
		}
		int i = 0;
		c.getPStats().doDeathsHS(true);
		sendFrame126("Name - Deaths", 6399);
		for (Object e : c.getPStats().theDeathsObject) {
			sendFrame126(1+i+". " + ((Map.Entry<String, Integer>) e).getKey() + " - " + ((Map.Entry<String, Integer>) e).getValue(), 6402+i);
			i++;
		}
	...
	public void buyPrayer(int amt, int price) {
		if (price > c.points) {
			c.sendMessage("You don't have enough points.");
			return;
		} else {
			c.points -= price;
			addSkillXP(amt, c.playerPrayer);
			c.refreshInfo();
			closeAllWindows();
			c.dialogueAction = -1;
			c.sendMessage("You bought "+amt+" Prayer XP for "+price+" points.");
		}
	}
	
	public void sellLogs() {
		int[] logs = {1511,1521,1519,1517,1515,1513}; //reg oak will map yew mag
		for (int i = 0; i < logs.length; i++) {
			if (c.getItems().playerHasItem(logs[i])) {
				int pointsToGive = c.getShops().getValue(logs[i])*c.getItems().getItemAmount(logs[i]);
				c.points += (int)pointsToGive/14;
				c.refreshInfo();
				c.sendMessage("You sold "+c.getItems().getItemAmount(logs[i])+" "+c.getItems().getItemName(logs[i])+" for "+(int)pointsToGive/14+" points.");
				c.getItems().deleteItem(logs[i], c.getItems().getItemAmount(logs[i]));
			} else {
				//c.sendMessage("You have no logs!");
			}
		}
		c.dialogueAction = 0;
	...
	public void sellOres() {
		int[] ores = {440,453,444,447,449,451}; //iron coal gold mith addy rune
		for (int i = 0; i < ores.length; i++) {
			if (c.getItems().playerHasItem(ores[i])) {
				int pointsToGive = c.getShops().getValue(ores[i])*c.getItems().getItemAmount(ores[i]);
				c.points += (int)pointsToGive/14;
				c.refreshInfo();
				c.sendMessage("You sold "+c.getItems().getItemAmount(ores[i])+" "+c.getItems().getItemName(ores[i])+" for "+(int)pointsToGive/14+" points.");
				c.getItems().deleteItem(ores[i], c.getItems().getItemAmount(ores[i]));
			} else {
				//c.sendMessage("You have no logs!");
			}
		}
	...
	public void duelNPC() {
		if (c.currentSeed == 0) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(1)); //man
			else
				startDuel(NPCRanks.getOpponentById(709)); //imp
		} else if (c.currentSeed == 1) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(13)); //wizard
			else
				startDuel(NPCRanks.getOpponentById(192)); //dark warrior
		} else if (c.currentSeed == 2) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(88)); //dungeon rat
			else
				startDuel(NPCRanks.getOpponentById(102)); //goblin
		} else if (c.currentSeed == 3) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(71)); //orc
			else
				startDuel(NPCRanks.getOpponentById(106)); //bear
		} else if (c.currentSeed == 4) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(15)); //warrior woman
			else
				startDuel(NPCRanks.getOpponentById(184)); //pirate
		} else if (c.currentSeed == 5) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(144)); //king scorpion
			else
				startDuel(NPCRanks.getOpponentById(191)); //tribesman
		} else if (c.currentSeed == 6) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(247)); //sir moldred
			else
				startDuel(NPCRanks.getOpponentById(237)); //renegade knight
		} else if (c.currentSeed == 7) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(753)); //melzar the mad
			else
				startDuel(NPCRanks.getOpponentById(190)); //zammy monk
		} else if (c.currentSeed == 8) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(1536)); //possessed pickaxe
			else
				startDuel(NPCRanks.getOpponentById(913)); //battle mage
		} else if (c.currentSeed == 9) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(116)); //cyclops
			else
				startDuel(NPCRanks.getOpponentById(145)); //ice warrior
		} else if (c.currentSeed == 10) {
			if (opponent())
				startDuel(NPCRanks.getOpponentById(374)); //ogre
			else
				startDuel(NPCRanks.getOpponentById(1593)); //wild dog
		} else if (c.currentSeed == 11) {
			startDuel(NPCRanks.getOpponentById(21)); //hero
		} else if (c.currentSeed == 12) {
			startDuel(NPCRanks.getOpponentById(1906));
		} else if (c.currentSeed == 13) {
			startDuel(NPCRanks.getOpponentById(277));
		} else if (c.currentSeed == 14) {
			startDuel(NPCRanks.getOpponentById(938));
		} else if (c.currentSeed == 15) {
			startDuel(NPCRanks.getOpponentById(1540));
		} else if (c.currentSeed == 16) {
			startDuel(NPCRanks.getOpponentById(1096));
		} else if (c.currentSeed == 17) {
			startDuel(NPCRanks.getOpponentById(1184));
		} else if (c.currentSeed == 18) {
			startDuel(NPCRanks.getOpponentById(477));
		} else if (c.currentSeed == 19) {
			startDuel(NPCRanks.getOpponentById(221));
		} else if (c.currentSeed == 20) {
			startDuel(NPCRanks.getOpponentById(1851));
		} else {
			c.sendMessage("Error!");
		}
	}

	public void startDuel(Opponent npc) {
		spellTeleport(2910, 3612, 0);
		c.npcDuelingWith = npc;
		c.sendMessage("Your fight has begun!");
		switch(npc.id) {
			case 1: //man
				Server.npcHandler.spawnOpponent(c, npc.id, 10, 1, 5, 5);
			break;
			case 709: //imp
				Server.npcHandler.spawnOpponent(c, npc.id, 8, 2, 12, 3);
			break;
			case 13: //wizard
				Server.npcHandler.spawnOpponent(c, npc.id, 18, 2, 25, 15);
			break;
			case 192: //dark warrior
				Server.npcHandler.spawnOpponent(c, npc.id, 20, 2, 20, 20);
			break;
			case 88: //dungeon rat
				Server.npcHandler.spawnOpponent(c, npc.id, 20, 3, 30, 20);
			break;
			case 102: //goblin
				Server.npcHandler.spawnOpponent(c, npc.id, 20, 3, 25, 25);
			break;
			case 71: //orc
				Server.npcHandler.spawnOpponent(c, npc.id, 30, 5, 40, 40);
			break;
			case 106: //bear
				Server.npcHandler.spawnOpponent(c, npc.id, 30, 5, 40, 40);
			break;
			case 15: //warrior woman
				Server.npcHandler.spawnOpponent(c, npc.id, 36, 5, 50, 55);
			break;
			case 184: //pirate
				Server.npcHandler.spawnOpponent(c, npc.id, 32, 6, 60, 40);
			break;
			case 144: //king scorpion
				Server.npcHandler.spawnOpponent(c, npc.id, 42, 6, 60, 50);
			break;
			case 191: //tribesman
				Server.npcHandler.spawnOpponent(c, npc.id, 32, 8, 85, 50);
			break;
			case 247: //sir moldred
				Server.npcHandler.spawnOpponent(c, npc.id, 50, 7, 70, 70);
			break;
			case 237: //renegade knight
				Server.npcHandler.spawnOpponent(c, npc.id, 50, 7, 80, 80);
			break;
			case 753: //melzar the mad
				Server.npcHandler.spawnOpponent(c, npc.id, 50, 8, 90, 65);
			break;
			case 190: //monk of zamorak
				Server.npcHandler.spawnOpponent(c, npc.id, 50, 10, 120, 60);
			break;
			case 1536: //pickaxe
				Server.npcHandler.spawnOpponent(c, npc.id, 40, 15, 180, 40);
			break;
			case 913: //battle mage
				Server.npcHandler.spawnOpponent(c, npc.id, 60, 13, 150, 90);
			break;
			case 116: //cyclops
				Server.npcHandler.spawnOpponent(c, npc.id, 65, 12, 135, 110);
			break;
			case 145: //ice warrior
				Server.npcHandler.spawnOpponent(c, npc.id, 50, 13, 150, 125);
			break;
			case 374: //ogre
				Server.npcHandler.spawnOpponent(c, npc.id, 70, 15, 180, 140);
			break;
			case 1593: //wild dog
				Server.npcHandler.spawnOpponent(c, npc.id, 50, 18, 200, 100);
			break;
			case 21: //hero
				Server.npcHandler.spawnOpponent(c, npc.id, 80, 17, 225, 225);
			break;
			case 1906: //tough guy
				Server.npcHandler.spawnOpponent(c, npc.id, 90, 20, 250, 200);
			break;
			case 277: //lesarkus
				Server.npcHandler.spawnOpponent(c, npc.id, 100, 24, 300, 175);
			break;
			case 938: //randalph devere
				Server.npcHandler.spawnOpponent(c, npc.id, 110, 22, 300, 180);
			break;
			case 1540: //treus
				Server.npcHandler.spawnOpponent(c, npc.id, 115, 25, 325, 200);
			break;
			case 1096: //stick
				Server.npcHandler.spawnOpponent(c, npc.id, 140, 27, 350, 300);
			break;
			case 1184: //elf warrior
				Server.npcHandler.spawnOpponent(c, npc.id, 140, 30, 400, 240);
			break;
			case 477: //khaz warlord
				Server.npcHandler.spawnOpponent(c, npc.id, 150, 28, 400, 325);
			break;	
			case 221: //black titan
				Server.npcHandler.spawnOpponent(c, npc.id, 200, 32, 400, 400);
			break;
			case 1851: //arzinian
				Server.npcHandler.spawnOpponent(c, npc.id, 255, 40, 625, 625);
			break;
			default:
			break;
		}
	...
	public void brawl(Opponent one, Opponent two, boolean bet) {
		if (bet) {
			if (c.points < c.playerBet) {
				return;
			} else {
				c.points -= c.playerBet;
				c.refreshInfo();
			}
		}
		int resultForOne = -1;
		int resultForTwo = -1;
		resultForOne = Misc.random(one.powerLevel);
		resultForTwo = Misc.random(two.powerLevel);
		if (resultForOne > resultForTwo) {
			c.winner = one;
			c.loser = two;
		} else if (resultForOne < resultForTwo) {
			c.winner = two;
			c.loser = one;
		} else {
			if (one.powerLevel > two.powerLevel) {
				c.winner = one;
				c.loser = two;
			} else if (one.powerLevel < two.powerLevel) {
				c.winner = two;
				c.loser = one;
			} else {
				c.winner = one;
		...
		Server.npcHandler.updateDeaths(c.loser.id);
		Server.npcHandler.updateKills(c.winner.id);
		if (bet) {
			if (c.chosenToWin.equals(c.winner)) {
				c.points += (c.playerBet*2);
				c.refreshInfo();
				c.getDH().sendDialogues(74, 518);
			} else {
				c.getDH().sendDialogues(75, 518);
			}
			c.betsMade++;
		}
	}
	
	public void clearArenaBetting() {
		c.fight = -1;
		c.playerBet = -1;
		c.chosenToWin = null;
		c.winner = null;
	...
	public void createDungeonOne() {
		c.sendMessage("Room 1 - Wait for monsters to spawn...");
		final Random r = new Random();
		Timer timer = new Timer();
		timer.schedule(new TimerTask() {
			@Override
			public void run() {
				c.sendMessage("Room 1 - Spawning...");
				for (int x = 0; x < 4; x++) {
					Server.npcHandler.spawnNpc(c, Misc.getRandomFromArray(Server.npcHandler.lowDungeon), r.nextInt((2425 - 2375) + 1) + 2375, r.nextInt((4685 - 4675) + 1) + 4675, 0, 1, 50, 5, 50, 50, true, false);
					Server.npcHandler.spawnNpc(c, Misc.getRandomFromArray(Server.npcHandler.medDungeon), r.nextInt((2425 - 2375) + 1) + 2375, r.nextInt((4685 - 4675) + 1) + 4675, 0, 1, 100, 12, 125, 125, true, false);
				}
				c.sendMessage("Room 1 - Spawned!");
			}			
		}, 5000);
	}
	...
	public void createDungeonTwo() {
		c.sendMessage("Room 2 - Wait for monsters to spawn...");
		final Random r = new Random();
		Timer timer = new Timer();
		timer.schedule(new TimerTask() {
			@Override
			public void run() {
				c.sendMessage("Room 2 - Spawning...");
				for (int x = 0; x < 4; x++) { //high
					Server.npcHandler.spawnNpc(c, Misc.getRandomFromArray(Server.npcHandler.highDungeon), r.nextInt((2425 - 2375) + 1) + 2375, r.nextInt((4735 - 4695) + 1) + 4695, 0, 1, 175, 25, 250, 250, true, false);
				}
				Server.npcHandler.spawnNpc(c, Misc.getRandomFromArray(Server.npcHandler.bossDungeon), 2384, 4721, 0, 1, 255, 50, 450, 450, true, false);
				Server.npcHandler.bossAlive = true;
				c.sendMessage("Room 2 - Spawned!");
			}			
		}, 5000);
	}
	...
	**/
	...
	public int pointsForDifficulty(int w, int l) {
		int diff = w - l;
		if (w == l) {
			return 15;
		} else if (diff > 0) { //126-125=1, 54-21=33, winner had higher level
			if (diff <= 5) {
				return 15;
		...
		} else if (diff < 0) { //120-125=-5, 44-66=-22, winner had lower level
			if (diff >= -5) {
				return 15;
			} else if (diff < -5 && diff >= -10) {
				return 20;
		...
		if(c.duelStatus != 7) {
			...
			if(o != null) {
				if (c.killerId != c.playerId) {
					o.playerKills++;
					o.playerStreak++;
					pointsGiven = pointsForDifficulty(o.combatLevel, c.combatLevel);
					o.points += pointsGiven;
					c.playerDeaths++;
					c.playerStreak = 0;
					o.sendMessage("You have defeated "+Misc.ucFirst(c.playerName)+"!");
					o.sendMessage("You have received "+pointsGiven+" points for your victory.");
					c.sendMessage("You have been defeated by "+Misc.ucFirst(o.playerName)+"!");
					c.sendMessage("You lose no points for losing to another player.");
					if (o.betsMade >= 5) {
						o.killsBeforeBet++;
						if (o.killsBeforeBet >= 10) {
							o.sendMessage("You are now eligible to bet on more arena matches.");
							o.killsBeforeBet = 0;
							o.betsMade = 0;
						}
					}
					if (o.playerKills >= 5 && o.pvpRank == 0) {
						o.sendMessage("Congratulations! You have achieved the Novice rank in the PvP Arena!");
						o.pvpRank++;
					}
					if (o.playerKills >= 25 && o.pvpRank == 1) {
						o.sendMessage("Congratulations! You have achieved the Veteran rank in the PvP Arena!");
						o.pvpRank++;
					}
					if (o.playerKills >= 100 && o.pvpRank == 2) {
						o.sendMessage("Congratulations! You have achieved the Reknowned rank in the PvP Arena!");
						o.pvpRank++;
					}
					if (o.playerKills >= 250 && o.pvpRank == 3) {
						o.sendMessage("Congratulations! You have achieved the Hero rank in the PvP Arena!");
						o.pvpRank++;
					}
					if (o.playerKills >= 500 && o.pvpRank == 4 && o.combatLevel == 126) {
						o.sendMessage("Congratulations! You have achieved the Legend rank in the PvP Arena!");
					...
					if (c.npcDuelingWith != null) {
						Server.npcHandler.updateKills(c.npcDuelingWith.id);
						c.npcDeaths++;
						c.npcStreak = 0;
						c.points -= 5;
						if (c.points < 0)
							c.points = 0;
						c.sendMessage("You have been defeated by a "+Misc.ucFirst(Server.npcHandler.getNpcListName(c.npcDuelingWith.id)).replace("_"," ")+"!");
						c.sendMessage("You have lost 5 points for losing to an NPC.");
						c.currentSeed = 0;
						c.refreshInfo();
						NPCHandler.npcs[c.npcDuelingWith.id] = null;
						c.npcDuelingWith = null;
						PlayerSave.saveGame(c);
						brawl(NPCRanks.getRandomOpponent(), NPCRanks.getRandomOpponent(), false);
					}
				}
	...
	**/
	...
	public void startTeleport(int x, int y, int height, String teleportType) {
		if (c.npcDuelingWith != null || c.inDungeonOne() || c.inDungeonTwo())
			return;
		if(c.duelStatus == 5) {
	...
	**/
	...
	public boolean fullRogue() {
		return c.playerEquipment[c.playerHat] == 5554 && c.playerEquipment[c.playerChest] == 5553 && c.playerEquipment[c.playerLegs] == 5555 && c.playerEquipment[c.playerHands] == 5556 && c.playerEquipment[c.playerFeet] == 5557;
	}
	public boolean fullVesta() {
		return c.playerEquipment[c.playerChest] == 13887 && c.playerEquipment[c.playerLegs] == 13893;
	}
	/**
	* reseting animation
	**/
...
if (c.expLock == true) {
	...
	public void addStarter() {
		...
		c.sendMessage("You have received 100 points to start out with!");
		c.sendMessage("Talk to the Wise Old Man to get started.");
		c.points += 100;
		c.refreshInfo();
	}
	...
	public void resetTzhaar() {
		...
		c.getPA().movePlayer(2398,5087, c.playerId * 4);
		c.waveId = 0;
		c.tzhaarToKill = -1;
		c.tzhaarKilled = -1;
