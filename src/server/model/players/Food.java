...
public class Food {
	...
	public void eatNPC(int id, int n) {
		if (NPCHandler.npcs[n] != null) {
			//Opponent o = NPCRanks.getOpponentById(c.npcDuelingWith);
			//System.out.println(c.npcDuelingWith.foodAmt+" foodamt");
			if (c.npcDuelingWith.foodId == -1)
				return;
			if (c.npcDuelingWith.foodAmt <= 0)
				return;
			//Server.npcHandler.startAnimation(animId, n);
			if (System.currentTimeMillis() - NPCHandler.lastEat >= 3000) {
				if (NPCHandler.npcs[n].HP > 0) {
					//reset attack?
					NPCHandler.npcs[n].gfx0(255);
					c.npcDuelingWith.foodAmt--;
					FoodToEat f = FoodToEat.food.get(id);
					if (NPCHandler.npcs[n].HP < NPCHandler.npcs[n].MaxHP) {
						NPCHandler.npcs[n].HP += f.getHeal();
						if (NPCHandler.npcs[n].HP > NPCHandler.npcs[n].MaxHP)
							NPCHandler.npcs[n].HP = NPCHandler.npcs[n].MaxHP;
					}
					NPCHandler.lastEat = System.currentTimeMillis();
					NPCHandler.npcs[n].updateRequired = true;
				}
			}
		} else {
