...
public class CombatAssistant{
	...
	*/
	...
	public void delayedHit(int i) { // npc hit delay
		...
		if (c.npcDuelingWith != null) {
			if (c.npcDuelingWith.id == NPCHandler.npcs[i].npcType) {
				if (NPCHandler.npcs[i].HP < (int)NPCHandler.npcs[i].MaxHP/2) {
					c.getFood().eatNPC(c.npcDuelingWith.foodId, i);
					NPCHandler.npcs[i].updateRequired = true;
				}
			}
		}
	...
	public void applyNpcMeleeDamage(int i, int damageMask) {
		...
		if (damage > 0 && c.getPA().fullRogue()) {
			damage = (int)((damage*110)/100);
		}
		NPCHandler.npcs[i].underAttack = true;
	...
	**/
	...
	public void applyPlayerMeleeDamage(int i, int damageMask){
		...
		if (damage > 0 && o.getPA().fullVesta()) {
			damage = (int)((damage*90)/100);
		}
		switch(c.specEffect) {
...
break; 
	...
	**/
	...
	public boolean checkReqs() {
		...
		if (c.inPits && PlayerHandler.players[c.playerIndex].inPits)
			return true;
		if(PlayerHandler.players[c.playerIndex].inDuelArenaLobby() && c.duelStatus != 5 && !c.usingMagic) {
	...
	**/
	...
	public void getPlayerAnimIndex(String weaponName){
		...
		if (weaponName.contains("wand") || weaponName.contains("staff") || weaponName.contains("spear") ) {
			c.playerStandIndex = 809;
			c.playerRunIndex = 1210;
			c.playerWalkIndex = 1146;
			return;
	...
	**/
	...
	public int getWepAnim(String weaponName) {
		...
		if(weaponName.endsWith("dagger") || weaponName.endsWith("rapier")) {
			return 412;
		}		
	...
	**/
	...
	public int getAttackDelay(String s) {
		...
		if(s.endsWith("greataxe"))
			...
			return 5;
		else if(s.equals("statius warhammer"))
			return 5;
		else if(s.equals("vestas spear"))
			return 5;
	...
	**/
	...
	public int calculateMeleeAttack() {
        ...
        if (c.prayerActive[2]) {
            ...
            attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.35;
        }
        if (c.fullVoidMelee())
