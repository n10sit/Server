...
import server.model.npcs.Opponent;
...
public abstract class Player {
	...
	public double playerKills = 0, playerDeaths = 0, npcKills = 0, npcDeaths = 0;
	public int playerStreak = 0, npcStreak = 0, points = 0;
	public Integer npcKillsInt = (int)npcKills;
	public Integer npcDeathsInt = (int)npcDeaths;

	public Opponent npcDuelingWith;
	public boolean chosenClass = false;
	public int npcRank = 0;
	...
	public int opponentFoodAmt = 0;
	
	/**public String[] fightOneName = new String[2];
	public int[] fightOnePower = new int[2];
	public String[] fightTwoName = new String[2];
	public int[] fightTwoPower = new int[2];
	public String[] fightThreeName = new String[2];
	public int[] fightThreePower = new int[2];**/
	
	public Opponent[] fightOne = new Opponent[2];
	public Opponent[] fightTwo = new Opponent[2];
	
	public int fight = -1;
	public int playerBet = -1;
	public Opponent chosenToWin = null;
	public Opponent winner = null;
	public Opponent loser = null;
	public int betsMade = 0;
	public int killsBeforeBet = 0;
	...
	public final int[] PRAYER_LEVEL_REQUIRED = 	{1,4,7,8,9,10,13,16,19,22,25,26,27,28,31,34,37,40,43,44,45,46,49,52,92,95};
	...
	public final String[] PRAYER_NAME = 		{"Thick Skin", "Burst of Strength", "Clarity of Thought", "Sharp Eye", "Mystic Will", "Rock Skin", "Superhuman Strength", "Improved Reflexes","Rapid Restore", 
												...
												"Protect from Melee","Eagle Eye", "Mystic Might", "Retribution", "Redemption", "Smite", "Soul Split", "Turmoil"};
	...
	public boolean inGWD() {
		if (absX > 2898 && absX < 2924 && absY > 3598 && absY < 3628) {
		return true;
		}
		return false;
	}
	...
	public boolean inDuelArenaLobby() {
		if(absX >= 3349 && absX <= 3400 && absY >= 3262 && absY <= 3280) {	
			return true;
		}
		return false;
	}
	...
	public boolean inDuelArena() {
		if((absX >= 3333 && absX <= 3388 && absY >= 3244 && absY <= 3258)) {
			return true;
		}
		return false;
	}
	
	public boolean inDungeonOne() {
		if (absX >= 2365 && absX <= 2435 && absY >= 4665 && absY <= 4690)
			return true;
		else
			return false;
	}
	
	public boolean inDungeonTwo() {
		if (absX >= 2365 && absX <= 2435 && absY >= 4690 && absY <= 4740)
			return true;
		else
			return false;
	}
	...
	public boolean inMulti() {
		if((absX >= 3136 && absX <= 3327 && absY >= 3519 && absY <= 3607) || 
			...
			(absX >= 2365 && absX <= 2435 && absY >= 4665 && absY <= 4690) ||
			(absX >= 2365 && absX <= 2435 && absY >= 4690 && absY <= 4740) ||
	...
	protected void appendPlayerAppearance(Stream str) {
		synchronized(this) {
			...
			combatLevel = (int) combatFormula(getLevelForXP(playerXP[0]), getLevelForXP(playerXP[2]), getLevelForXP(playerXP[1]), getLevelForXP(playerXP[3]), getLevelForXP(playerXP[5]), getLevelForXP(playerXP[4]), getLevelForXP(playerXP[6]));

			playerProps.writeByte(combatLevel);		// combat level		
			playerProps.writeWord((int)npcKills);
			playerProps.writeWord((int)playerKills);
			str.writeByteC(playerProps.currentOffset);		
			str.writeBytes(playerProps.buffer, playerProps.currentOffset, 0);
		}
	}
	
	public double combatFormula(int attack, int strength, int defence, int hitpoints, int prayer, int ranged, int magic) {
		double level = (defence + hitpoints + Math.floor(prayer / 2) + Math.floor(1 / 2)) * 0.25;
		double warrior = (attack + strength) * 0.325;
		double ranger = ranged * 0.4875;
		double mage = magic * 0.4875;
		return level + Math.max(warrior, Math.max(ranger, mage));
	}
